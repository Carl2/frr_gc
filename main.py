...
def plot_rider_times(rider_struct, names, stages):
    ...
    for name in names:
        times = create_array_from_struct(rider_struct,
                                         name,
                                         stages,
                                         'Time',
                                        error_fn=lambda : np.array(['nat'], dtype='datetime64')
                                         )
        egaps = create_array_from_struct(rider_struct,
                                         name,
                                         stages,
                                         'Egap',
                                         error_fn=lambda : np.array([timedelta(seconds=0)])
                                         )
